...
function desenharGraficoPorNivel() {
    ...
    let html = `
        <div class="level-chart">
            <div class="level-chart-title">📊 Desempenho por nível</div>
            <div class="level-bar-group">
    `;

    for (const ano of anos) {
        const stats = statsPorNivel[ano] || { acertos: 0, erros: 0 };
        const total = stats.acertos + stats.erros;
        const percAcertos = total > 0 ? (stats.acertos / total) * 100 : 0;
        const percErros = total > 0 ? (stats.erros / total) * 100 : 0;

        html += `
            <div class="level-bar-row">
                <div class="level-bar-label">
                    <span class="${ano.replace(' ', '-').toLowerCase()}-label">${labels[ano]}</span>
                </div>
                <div class="level-bar-track">
                    <div class="level-bar-fill-acertos" style="width: ${percAcertos}%;"></div>
                    <div class="level-bar-fill-erros" style="width: ${percErros}%;"></div>
                </div>
                <div class="level-bar-numbers">
                    <span class="acert-count">${stats.acertos}</span>/
                    <span class="err-count">${stats.erros}</span>
                    <span style="color:#666; font-weight:400;">(${total})</span>
                </div>
            </div>
        `;
    }
